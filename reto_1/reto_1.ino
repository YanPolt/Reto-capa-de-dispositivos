...
void loop(void) { 
  ...
  //El mensaje que se envía es de la forma {"value": x}, donde x es el valor de Iluminance
  
  //JSON para Iluminance
  String json = "{\"value\": "+ String(Iluminance) + "}";
  ...
  if ( !isnan(Iluminance) && !isnan(LDR_Val) ) {
    //Publica en el tópico de la Luminosidad
    client.publish(MQTT_PUB_TOPIC3, payload1, false);
  }
